...
export class RetailService {
  ...
  async updateRetailUser(data: UpdateUserRequest) {
    try {
      ...
      if (!user) {
        return handleError('User not found', null);
      }
      ...
      if (data.username && data.username !== user.username) {
        ...
        if (existingUser) {
          return handleError('Username already exists', null);
        }
      }
      ...
      const updatedUser = await this.prisma.$transaction(async (prisma) => {
        ...
        if (data.password) {
          updateData.password = this.jwtService.encodePassword(data.password);
        }
        ...
        if (
          ...
          await prisma.userDetails.update({
            ...
            data: {
              firstName: data.firstName || user.userDetails.firstName,
              lastName: data.lastName || user.userDetails.lastName,
              email: data.email || user.userDetails.email,
              phone: data.phoneNumber || user.userDetails.phone,
              address: data.address || user.userDetails.address,
              city: data.city || user.userDetails.city,
              lga: data.lga || user.userDetails.lga,
              state: data.state || user.userDetails.state,
              language: data.language || user.userDetails.language,
              country: data.country || user.userDetails.country,
              gender: data.gender || user.userDetails.gender,
              currency: data.currency || user.userDetails.currency,
              date_of_birth: data.dateOfBirth || user.userDetails.date_of_birth,
            },
          });
        }
      ...
      return handleResponse(updatedUser, 'Shop User updated successfully');
    } catch (error) {
      return handleError(error.message, error);
    }
  }
  ...
  async listAgentUsers(payload: GetAgentUsersRequest) {
    try {
      ...
      const agent: any = await this.prisma
        .$queryRaw`SELECT u.clientId, u.id, u.username, u.code, u.role_id, ud.firstName, ud.lastName, ud.phone as phone_number, ud.email, r.name as rolename FROM users u
  ...
  async listAgents(data: GetAgentUsersRequest) {
    try {
      ...
      const roles = await this.prisma.role.findMany({
        where: { type: 'agency', name: { not: 'Cashier' } },
        select: { id: true },
      });
        ...
        for (const agent of agents) {
          ...
          if (agentUsers.length) {
            ...
            if (salesRes.success) {
              ...
              total.noOfBets =
                total.noOfBets + parseFloat(salesRes.data?.noOfBets || 0);
              total.totalStake =
                total.totalStake + parseFloat(salesRes.data?.totalStake || 0);
              total.totalWinnings =
                total.totalWinnings +
                parseFloat(salesRes.data?.totalWinnings || 0);
